...
unbound = lambda x: jnp.log(jnp.expm1(x))  # ln(e^x - 1)
...
def spec2vec(spec, filter):
    ...
    paramflat = jnp.asarray(paramflat)  # NOTE: paramflat will zero-dim array entering and leaving JAX
...
def spectral_loss(paramflat, paramdef, static, spectral_density, m, V, dt, clip=1e-5):
    def kernel_loss(kernel_spec, freq):
        ...
        return -.5 * jnp.sum(jnp.log(Sw) + Zw / Sw)
    ...
    log_p_s = jax.tree_map(partial(kernel_loss, freq=freq), latent_spec, is_leaf=lambda x: isinstance(x, dict))
...
def whittle(latent_spec, filter, m, V, dt, clip=1e-5):
    ...
    opt = optimize.minimize(spectral_loss, x0=paramflat, args=(paramdef, static, spectral_density, m, V, dt, clip), method='BFGS')
    paramflat = opt.x
    # bound hyperparams
